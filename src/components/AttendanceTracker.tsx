...
import { LogIn, LogOut, UserX, UserPlus, Trash2 } from 'lucide-react';

export function AttendanceTracker() {
  const { employees, addEmployee, removeEmployee, checkIn, checkOut, markAbsent, getTodayRecord } = useAttendance();
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showAddForm && (
        <div className="glass-card p-4 flex gap-3 items-end">
          ...
          <Button onClick={handleAdd} className="gradient-orange text-primary-foreground">
            Guardar
          </Button>
        </div>
      )}
              ...
              <div className="flex-1">
                ...
                {record && (
                  <div className="flex gap-3 mt-1 text-xs">
                    {record.checkIn && (
                      <span className="text-success">Entrada: {record.checkIn}</span>
                    )}
                    {record.checkOut && (
                      <span className="text-primary">Salida: {record.checkOut}</span>
                    )}
                    {isAbsent && (
